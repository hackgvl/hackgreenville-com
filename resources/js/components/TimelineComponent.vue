        ...
        <div class="timeline-panel">
          <div class="timeline-heading">
            <h4 class="timeline-title">
              <span v-if="event.cancelled" class="text-danger"
                >[CANCELLED]
              </span>
              {{ event.title }}
            </h4>
...
export default  defineComponent({
  ...
  methods: {
    async load() {
      this.loading++;
      const { data: events = [] } =
        (await window.axios.get(this.event_data_route)) || {};
      this.events = events;
      this.loading--;
    },
    showMore({
      ...
      const desc =
        ...
        `</div>` +
        `<div class="text-left">` +
        `<strong>Starts:</strong> ${active_at_ftm} on ${moment(
          active_at,
        ).format(datetime_format)}` +
        '<br /><br />' +
        (cancelled
          ? '<h3 class="text-danger">This event has been cancelled</h3><br />'
          : '') +
        description +
        '</div>';
