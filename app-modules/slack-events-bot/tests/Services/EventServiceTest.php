...
class EventServiceTest extends DatabaseTestCase
{
    ...
    #[Test]
    public function it_generates_blocks_with_null_organization(): void
    {
        $event = Event::factory()->create([
            'event_name' => 'Event Without Org',
            'description' => 'Description for event without organization.',
        ...
        $this->assertEquals('*Unknown Organization*', $blocks[1]['fields'][0]['text']);
    }

    #[Test]
    public function it_generates_text_with_null_organization(): void
    {
        $event = Event::factory()->create([
            'event_name' => 'Event Without Org',
        ...
        $this->assertStringContainsString('Unknown Organization', $generatedText);
    }

    #[Test]
    public function it_generates_text_with_sanitized_url(): void
    {
        $organization = Org::factory()->create(['title' => 'Test Organization']);

        $event = Event::factory()->create([
            'event_name' => 'Event With Bad URL',
            'description' => 'Description.',
            'organization_id' => $organization->id,
            'venue_id' => null,
            'uri' => 'https://example.com/event>|malicious',
            'active_at' => now()->addDays(7),
        ]);

        $generatedText = $this->eventService->generateText($event);

        $this->assertStringNotContainsString('>', $generatedText);
        $this->assertStringNotContainsString('|', $generatedText);
        $this->assertStringContainsString('https://example.com/eventmalicious', $generatedText);
    }
